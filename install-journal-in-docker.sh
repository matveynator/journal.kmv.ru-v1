...
# Default values
default_backup_dir="/backup/journal"
...
docker volume rm journal_mysql_data journal_site_data &> /dev/null
docker volume create --opt type=none --opt device="$backup_dir/mysql" --opt o=bind journal_mysql_data
docker volume create --opt type=none --opt device="$backup_dir/data" --opt o=bind journal_site_data
...
docker run -d -p "$host_port:80" \
    ...
    -v journal_mysql_data:/var/lib/mysql \
    -v journal_site_data:/opt/journal \
    -e MAILHUB="$mailhub" \
    -e EMAIL="$email" \
    -e DOMAIN="$domain" \
